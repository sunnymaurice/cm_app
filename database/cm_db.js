module.exports = exports = (function export_cm_db() {
	var mysql = require('mysql');
	var mydb_setting = require('../config/db_settings.json');

	var createDB_sql = 'CREATE DATABASE IF NOT EXISTS '+ mydb_setting.DEFAULT_DB;
	var useDB_sql = 'USE ' + mydb_setting.DEFAULT_DB;

	var createTABLE_sql = 
		'CREATE TABLE IF NOT EXISTS '+ mydb_setting.DEFAULT_TABLE +
		' (' +
	...
	var createNewPool = function createNewPool() {

		var pool = mysql.createPool({
			connectionLimit: mydb_setting.CONNECTION_LIMIT,
			host: mydb_setting.DEFAULT_HOST,
			port: mydb_setting.DEFAULT_PORT,
			user: mydb_setting.DEFAULT_USER,
			password: mydb_setting.DEFAULT_PASSWD,
			database: mydb_setting.DEFAULT_DB,
			//insecureAuth: false,
			trace: true,
			debug: false  //set true for debugging trace 
		});

		console.log('Create a new mysql pool on: ' + mydb_setting.DEFAULT_HOST + ', port :' + mydb_setting.DEFAULT_PORT);

		return pool;  
	};

	var db_init = function db_init(myPool) {
	...
	*/
	...
	var endConnections = function endConnections(myPool) {
	 ...
	 *  TODO:
	 */
	var insertData = function insertData(myPool, tableName, record) {
	...
	function deleteData(myPool, tableName, delField) {
		...
		//var delete_sql = 'DELETE FROM '+ mydb_setting.DEFAULT_TABLE +' WHERE id = \'213012345\''; 
	...
	var dropTable = function dropTable(myPool, tableName) {
