module.exports = exports = (function export_cm_proto_parser() {
	
	var dp3710_pkt_format = require('../config/dp3710_protocol.json');
	...
	function transformDbTime(timeStr) {
		...
		if (timeStr.length == 4) {
			// hh:mm format of old DP3710 and MS3500
			return timeStr.substr(0, 2) + ':' + timeStr.substr(2, 2);
		} else if (timeStr.length == 6) {
			// hh:mm:ss format of MS3510, DP3710
	...
	function DP3710parser(pkt_str) {
		...
		var decimalPlace = parseInt(pkt_str.substr(dp3710_pkt_format.DEC_PLACE_OFFSET, dp3710_pkt_format.DEC_PLACE_LEN));

		var modelNum = 	pkt_str.substr(dp3710_pkt_format.MODEL_OFFSET, dp3710_pkt_format.MODEL_LEN);
		modelNum = modelNum.trim(modelNum);
		var SN = pkt_str.substr(dp3710_pkt_format.SN_OFFSET, dp3710_pkt_format.SN_LEN);
		var ID = pkt_str.substr(dp3710_pkt_format.ID_OFFSET, dp3710_pkt_format.ID_LEN);
		var gWeight = parseInt(pkt_str.substr(dp3710_pkt_format.GROSS_WEIGHT_OFFSET, dp3710_pkt_format.GROSS_WEIGHT_LEN))/Math.pow(10, decimalPlace);	
		var tWeight = parseInt(pkt_str.substr(dp3710_pkt_format.TARE_WEIGHT_OFFSET, dp3710_pkt_format.TARE_WEIGHT_LEN))/Math.pow(10, decimalPlace);
		var nWeight = parseInt(pkt_str.substr(dp3710_pkt_format.NET_WEIGHT_OFFSET, dp3710_pkt_format.NET_WEIGHT_LEN))/Math.pow(10, decimalPlace);
		var heightStr = pkt_str.substr(dp3710_pkt_format.HEIGHT_OFFSET, dp3710_pkt_format.HEIGHT_LEN);
		var height;
		if (heightStr !== '----') {
		...
		var bmiStr = pkt_str.substr(dp3710_pkt_format.BMI_OFFSET, dp3710_pkt_format.BMI_LEN);
		var BMI;
		if (bmiStr !== 'N/A') {
		...
		//console.log("BMI: "+BMI);	
		var date = transformDbDate(pkt_str.substr(dp3710_pkt_format.DAY_OFFSET, dp3710_pkt_format.DAY_LEN));
		//console.log('Date:' + date);
		var time = transformDbTime(pkt_str.substr(dp3710_pkt_format.TIME_OFFSET, dp3710_pkt_format.TIME_LEN));
	...
	/* 
		...
		Return Value: 	
	 */
	var hex2String = function hex2String(pkt_data) {
		var pkt_str;
		var i;
		for(i in pkt_data){
	...
	/* 	
		...
		Return Value: 
	 */
	var pktInterpreter = function pktInterpreter(pkt_str) {
